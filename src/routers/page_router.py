...
async def home(request: Request, user: User = Depends(get_current_user)):
    ...
    return templates.TemplateResponse(
        request=request, name="pages/home.html", context={"path": path, "user": user}
    )
...
async def browse(request: Request, user: User = Depends(get_current_user_or_redirect)):
    path = request.url.path
    return templates.TemplateResponse(
        request=request, name="pages/browse.html", context={"path": path, "user": user}
    )
...
async def unauthorised(request: Request, old_path: str = Query("old_path"), user: User = Depends(get_current_user)):
    return templates.TemplateResponse(
        request=request, name="pages/unauthorised.html", context={"path": old_path, "user": user}
    )